...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dims, h_dims) -> None:
        ...
        self.heads = nn.ModuleList(
            [nn.Linear(h_dims[-1], out_dims_flat[i]) for i in range(len(out_dims))]
        )

    def forward(self, x, y):  # TODO naming
        ...
        for i, (o, d) in enumerate(zip(head_out, self.out_dims)):
            out_o = o.reshape(o.shape[0], *d)
            out.append(out_o)

        leaf_params = torch.cat(out[:2], dim=-1)
        # leaf_params = leaf_params.reshape((*leaf_params.shape[:-2], leaf_params.shape[-2] * leaf_params.shape[-1]))
...
class CNN(nn.Module):
    def __init__(self, out_dims) -> None:
        ...
        self.heads = nn.ModuleList(
            [nn.Linear(128, out_dims_flat[i]) for i in range(len(out_dims))]
        )
