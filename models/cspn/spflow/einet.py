...
class ConditionalEinet(Module, Classifier):
    """Conditional Einsum Network (Einet) for scalable deep probabilistic modeling.
    Code is taken from SPFlow's Einet implementation and adapted to fit our CSPN framework.
    ...
    def __init__(
        ...
        if layer_type not in ("einsum", "linsum"):
            raise InvalidParameterError(
                f"layer_type must be 'einsum' or 'linsum', got {layer_type}"
            )
        if structure not in ("top-down", "bottom-up"):
            raise InvalidParameterError(
                f"structure must be 'top-down' or 'bottom-up', got {structure}"
            )
    ...
    def _has_partial_evidence(data: torch.Tensor) -> bool:
        ...
        return bool(
            torch.isnan(data).any().item() and torch.isfinite(data).any().item()
        )
